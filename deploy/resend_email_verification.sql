-- Deploy postgraphile_user_system:resend_email_verification to pg
-- requires: users
...
CREATE OR REPLACE FUNCTION resend_email_verification_code (
  email_id uuid) RETURNS boolean AS
$$
begin
  if exists(
    select 1
